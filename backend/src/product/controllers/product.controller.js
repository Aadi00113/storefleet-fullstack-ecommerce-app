...
export const getAllProducts = async (req, res, next) => {
  try {
    ...
    // Filter by price — convert strings to numbers for proper numeric comparison
    if (price) {
      const priceQuery = {};
      if (price.gte !== undefined) priceQuery.$gte = Number(price.gte);
      if (price.lte !== undefined) priceQuery.$lte = Number(price.lte);
      if (Object.keys(priceQuery).length) query.price = priceQuery;
    }

    // Filter by rating — convert strings to numbers
    if (rating) {
      const ratingQuery = {};
      if (rating.gte !== undefined) ratingQuery.$gte = Number(rating.gte);
      if (rating.lte !== undefined) ratingQuery.$lte = Number(rating.lte);
      if (Object.keys(ratingQuery).length) query.rating = ratingQuery;
    }
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    // Guard against NaN when all reviews are deleted
